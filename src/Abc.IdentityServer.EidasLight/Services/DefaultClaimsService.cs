...
#pragma warning restore SA1401 // Fields should be private
        ...
        {
            Logger.LogDebug(
                "Getting claims for eIDAS light token for subject: {subject} and client: {clientId}",
                validatedRequest.Subject.GetSubjectId(),
                validatedRequest.Client.ClientId);

            var ctx = new ProfileDataRequestContext(validatedRequest.Subject, validatedRequest.Client, "eIDASLight", requestedClaimTypes)
            {
